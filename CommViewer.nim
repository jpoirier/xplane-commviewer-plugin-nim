...
proc CommandHandler(inCommand: XPLMCommandRef, inPhase: XPLMCommandPhase,
  inRefcon: pointer): int {.exportc: "CommandHandler", dynlib.} =
...
proc DrawWindowCallback(inWindowID: XPLMWindowID, inRefcon: pointer)
  {.exportc: "DrawWindowCallback", dynlib.} =
  ...
  of COMMVIEWER_WINDOW:
      ...
      rx_status = if (if cast[bool](pilotedge_rx_status_dataref):
        cast[bool](XPLMGetDatai(pilotedge_rx_status_dataref)) else: false): "1" else: "0";
      tx_status = if (if cast[bool](pilotedge_tx_status_dataref):
        cast[bool](XPLMGetDatai(pilotedge_tx_status_dataref)) else: false): "1" else: "0";
      connected = if (if cast[bool](pilotedge_connected_dataref):
        cast[bool](XPLMGetDatai(pilotedge_connected_dataref)) else: false): "YES" else: "NO "
      ...
      var str1: cstring = "[PilotEdge] Connected: $1 \t\t\tTX: $2\t\t\tRX: $3" %
                          [connected, tx_status, rx_status]
